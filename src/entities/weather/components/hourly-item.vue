...
import { isNewDay } from '~/src/shared/utils/date-time/is-new-day'
import { getWeekday } from '~/src/shared/utils/date-time/get-weekday'
import { getTime } from '~/src/shared/utils/date-time/get-time'

defineProps<{
  weatherItem: Forecast
  hourlyForecast: DailyForecast | null
  index: number
}>()
</script>
...
<template>
  <div
    class="hourly__item item"
    :class="{ item__newDay: isNewDay(index, hourlyForecast) }"
  >
    <div
      class="item__time"
      :class="{ 'item--newDayTime': isNewDay(index, hourlyForecast) }"
    >
      {{
        weatherItem.type === 'sunrise' || weatherItem.type === 'sunset' ? weatherItem.time : isNewDay(index, hourlyForecast) ? getWeekday(weatherItem.dt_txt) + ', ' + getTime(weatherItem.dt_txt) : getTime(weatherItem.dt_txt)
      }}
    </div>
    <div class="item__icon">
      <img
        class="icon"
        :src="(weatherItem.type === 'sunrise'
